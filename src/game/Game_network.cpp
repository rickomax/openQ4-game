...
// RAVEN END
	...
	// ClientReadSnapshot, so anything still queued because its server timestamp had
	// not been reached yet waits a whole snapshot interval - 50ms at the default
	// net_serverSnapshotDelay - before it fires.  That is a client behind the
	// server, which is exactly when its combat feedback is already thin.  The queue
	// is time ordered and the drain stops at the first event in the future, so
	// running it again here only ever releases events whose time has come.
	if ( isNewFrame ) {
		ClientProcessEntityNetworkEventQueue();
	}
